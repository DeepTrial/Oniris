/**
 * Netron Grapher - Browser Compatible Version
 * Modified from Netron's grapher.js for use in Oniris
 */
...
grapher.Graph = class {
    ...
    build(document) {
        ...
        if (!origin) {
            throw new Error("Origin element not found");
        }
    ...
    async layout(worker) {
        ...
        if (worker) {
            ...
            if (typeof dagre !== 'undefined') {
                ...
                console.error('dagre not loaded');
                return;
            }
        }
...
grapher.Edge = class {
    ...
    build(document, edgePathGroupElement, edgePathHitTestGroupElement, edgeLabelGroupElement) {
        ...
        this.element.setAttribute('fill', 'none');  // Prevent fill
        this.element.setAttribute('marker-end', 'url(#arrowhead)');  // Add arrow marker
        edgePathGroupElement.appendChild(this.element);
        this.hitTest = createElement('path');
        this.hitTest.setAttribute('fill', 'none');  // Prevent fill
